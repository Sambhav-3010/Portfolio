...
import { getLeetCodeLevelClass } from "@/utils/leetcode"
          ...
          <div className="flex mb-3">
            ...
            <div className="flex gap-1 flex-1 justify-between">
              {viewWeeks.map((_, index) => (
                <div key={index} className="flex-1 min-w-2.5 text-[10px] text-muted-foreground font-semibold">
                  {labels[index]}
                </div>
              ))}
            </div>
                  ...
                  {week.map((day, dayIndex) => {
                    ...
                    return (
                      <div key={dayIndex} className="group relative w-full">
                        <div
                          className={`aspect-square w-full rounded-[3px] transition-all group-hover:ring-2 group-hover:ring-primary/45 ${getLeetCodeLevelClass(
                            day.submissionCount
                          )}`}
                        />
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex gap-0.75">
            {[0, 1, 3, 6, 10].map((count) => (
              <div key={count} className={`h-3 w-3 rounded-[2px] ${getLeetCodeLevelClass(count)}`} />
            ))}
          </div>
          <span>More</span>
        </div>
